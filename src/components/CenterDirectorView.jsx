...
export default function CenterDirectorView({
  ...
  // Build flatLiveData so DataEntryTab flat keys (liveData.data) always win.
  // Spread nested section objects first, then liveData.data last — last-write-wins ensures
  // values entered via DataEntryTab override any stale nested section data.
  const _ld = liveData || {};
  const _sections = Object.entries(_ld)
    .filter(([k, v]) => k !== 'data' && typeof v === 'object' && !Array.isArray(v))
    .map(([, v]) => v);
  const flatLiveData = Object.assign({}, ..._sections, _ld.data || {});
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100%' }}>
      ...
      <div className="content">
        {activeTab==='overview'   && <OverviewTab    center={centerWithSeed} scoreColor={scoreColor} scoreLabel={scoreLabel} liveData={flatLiveData} reg={reg} onNavigate={(tab, domainNum) => { setInitialDomain(domainNum || null); setActiveTab(tab === 'dataentry' ? 'dataentry' : 'inspection'); }}/>}
