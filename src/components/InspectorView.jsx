...
function getDomainFields(center,reg,liveData){
  ...
  return [
    {id:'d1',label:'Licensing & Administration',dNum:'D1',color:D_COLORS[0],fields:[
      ...
      {id:'insurancePolicyNum',label:'GL coverage expiry',value:v(lic.insurancePolicyNum)},
      {id:'insuranceExpiry',label:"Workers' comp current",value:v(lic.insuranceExpiry)},
      {id:'workersCompExpiry',label:"Workers' comp expiry",value:v(lic.workersCompExpiry)},
      {id:'lastInspectionDate',label:'Last licensing inspection date',value:v(lic.lastInspectionDate)},
      {id:'lastInspectionResult',label:'Last inspection result',value:v(lic.lastInspectionResult)},
      {id:'qrisStatus',label:'QRIS enrolled',value:v(lic.qrisStatus),note:reg?.qrs||''},
      {id:'coiOnFile',label:'Certificate of Insurance (COI) current',value:v(lic.coiOnFile)},
    ]},
    {id:'d2',label:'Physical Environment',dNum:'D2',color:D_COLORS[1],fields:[
      {id:'indoorSqft',label:'Total indoor sq ft',value:v(phy.indoorSqft),note:`Req: ${reg?.indoorSqft||'?'} sq ft/child`},
      {id:'outdoorSqft',label:'Total outdoor sq ft',value:v(phy.outdoorSqft),note:`Req: ${reg?.outdoorSqft||'?'} sq ft/child`},
      {id:'capacity',label:'Licensed capacity (max children)',value:v(phy.capacity)},
      {id:'coDetectorInstalled',label:'CO detectors installed',value:v(phy.coDetectorInstalled)},
      {id:'smokeDetectorInstalled',label:'Smoke detectors installed',value:v(phy.smokeDetectorInstalled)},
      {id:'fireExtinguisherCurrent',label:'Fire extinguishers current',value:v(phy.fireExtinguisherCurrent)},
      {id:'firstAidKitPresent',label:'First aid kit accessible — fully stocked',value:v(phy.firstAidKitPresent)},
      {id:'hotWaterMaxTemp',label:'Hot water temperature',value:v(phy.hotWaterMaxTemp),note:`Max ${rules.hotWaterMax||110}°F`},
      {id:'fencingEnclosesPlayArea',label:'Gate self-latching',value:v(phy.fencingEnclosesPlayArea)},
      {id:'fencingHeightFt',label:'Fencing height (ft)',value:v(phy.fencingHeightFt),note:`Min ${rules.minFencingHeight||'4 ft'}`},
      {id:'toiletCompliant',label:'Child-accessible toilets (count)',value:v(phy.toiletCompliant),note:rules.toiletRatio?`1:${rules.toiletRatio}`:''},
    ]},
    {id:'d3',label:'Personnel & Qualifications',dNum:'D3',color:D_COLORS[2],fields:[
      {id:'directorName',label:'Director qualification pathway',value:v(per.directorName)},
      {id:'directorEduLevel',label:'Director qualification pathway',value:v(per.directorEduLevel),note:reg?.directorReq||''},
      {id:'directorYearsExp',label:'Director credential on file',value:v(per.directorYearsExp)},
      {id:'teacherEduMeetsReq',label:'Lead teacher qualification met',value:v(per.teacherEduMeetsReq),note:reg?.teacherReq||''},
      {id:'teacherMinAgeCompliant',label:'Aide age requirement met (18+)',value:v(per.teacherMinAgeCompliant)},
      {id:'bgCheckType',label:'BG check complete — all staff',value:v(rules.bgCheckType)},
      {id:'bgValid',label:'State BG check clearance on file',value:v(cr.bgValid),note:`of ${cr.bgTotal||'?'} total staff`},
      {id:'fbiClearance',label:'FBI fingerprint clearance on file',value:v(per.fbiClearance)},
      {id:'childAbuseRegistry',label:'Child abuse registry check',value:v(per.childAbuseRegistryDate)},
      {id:'adminDesignationOnFile',label:'Director on duty policy on file',value:v(per.adminDesignationOnFile)},
      {id:'workforceRegistry',label:'Workforce registry enrolled',value:v(cr.workforceRegistryDone),note:rules.workforceRegistry||''},
    ]},
    ...
    {id:'d5',label:'Staff Health & Training',dNum:'D5',color:D_COLORS[4],fields:[
      {id:'cprCertValid',label:'CPR certification on file — required staff',value:v(h.cprCertValid),note:`Renewal: ${rules.cprRenewal||'2 years'}`},
      {id:'cprExpiryDate',label:'CPR certification date on file',value:v(h.cprExpiryDate)},
      {id:'firstAidCertValid',label:'First aid certification current',value:v(h.firstAidCertValid)},
      {id:'tbScreeningAllStaff',label:'TB screening complete — all staff',value:v(h.tbScreeningAllStaff),note:rules.tbTestReq||''},
      {id:'physicalExamOnFile',label:'Staff physical exam on file — all staff',value:v(h.physicalExamOnFile)},
      {id:'trainingHrs',label:'Annual training hours completed',value:v(cr.trainingHrs),note:`Req: ${reg?.trainingHrs||'?'} hrs/yr`},
      {id:'mandatedReporterDone',label:'Mandated reporter training complete',value:v(cr.mandatedReporterDone),note:rules.mandatedReporterRenewal||''},
      {id:'newHireOrientation',label:'Orientation complete',value:v(h.newHireOrientationCompleted)},
      {id:'trainingLogOnFile',label:'Training log on file',value:v(h.trainingLogOnFile)},
      {id:'tbRenewalDueDate',label:'TB test date on file',value:v(h.tbRenewalDueDate)},
    ]},
    {id:'d6',label:"Children's Records & Health",dNum:'D6',color:D_COLORS[5],fields:[
      {id:'childRecordComplete',label:'Child enrollment record complete — all children',value:v(ch.childRecordComplete)},
      {id:'emergContactsOnFile',label:'Emergency contacts on file — all children',value:v(ch.emergContactsOnFile)},
      {id:'authPickupOnFile',label:'Authorized pickup list on file — all children',value:v(ch.authPickupOnFile)},
      {id:'allergyDocOnFile',label:'Allergy documentation on file — all children',value:v(ch.allergyDocOnFile)},
      {id:'allergyCareplan',label:'Allergy care plan on file',value:v(ch.allergyCareplan)},
      {id:'medLogMaintained',label:'Medication administration log maintained',value:v(ch.medLogMaintained)},
      {id:'medsStoredCorrectly',label:'Medications stored correctly',value:v(ch.medsStoredCorrectly)},
      {id:'immRecordsOnFile',label:'Immunization records on file — all children',value:v(ch.immRecordsOnFile),note:`Exemptions: ${rules.immExemptions||'Medical only'}`},
      {id:'immRecordsCurrent',label:'Immunization records current',value:v(ch.immRecordsCurrent)},
      {id:'parentAgreementSigned',label:'Parent / guardian agreement signed — all children',value:v(ch.parentAgreementSigned)},
      ...
      {id:'attendanceRetentionMet',label:'Incident / accident log on file',value:v(ch.attendanceRetentionMet),note:`Retain ${rules.recordRetention||'3 years'}`},
    ]},
    {id:'d7',label:'Emergency & Safety',dNum:'D7',color:D_COLORS[6],fields:[
      ...
      {id:'fireEvacPosted',label:'Fire evacuation plan posted visibly',value:v(em.fireEvacPosted)},
      ...
      {id:'fireDeptInspCurrent',label:'Fire department inspection current',value:v(em.fireDeptInspCurrent)},
      {id:'tornadoDrillDate',label:'Last tornado drill date',value:v(em.tornadoDrillDate),note:rules.tornadoDrill||'Check state'},
      {id:'lockdownDrillDate',label:'Last lockdown drill date',value:v(em.lockdownDrillDate),note:rules.lockdownDrill||'2x/year'},
      {id:'emergencyPlanOnFile',label:'Written emergency plan on file',value:v(em.emergencyPlanOnFile)},
      {id:'emergencyPlanPosted',label:'Emergency plan reviewed annually',value:v(em.emergencyPlanPosted)},
      {id:'drillLogMaintained',label:'All drill logs retained',value:v(em.drillLogMaintained)},
      {id:'annualHealthInsp',label:'Health department inspection current',value:v(em.annualFireInspOnFile)},
      {id:'bodiesOfWater',label:'Water safety plan on file (if pool / water feature)',value:v(em.bodiesOfWaterOnPremises)},
    ]},
  ];
}
